    ...
    if (tooltipModel.body) {
      ...
      if (selectedBarElement > -1) {
        ...
        titleLines.forEach(function (title) {
          ...
          if (selectedBarElement > -1) {
            ...
            innerHtml += `<tr class='border-b-2 border-solid border-primary text-xs'><td class='text-secondary flex flex-row items-center mx-2 pb-4'>
                                            <div class='w-2 h-2 rounded-full mr-2.5 border-solid' style="background-color: ${
            ...
            innerHtml += `<td class='text-secondary text-right pr-5 mx-1 pb-4'>${formattedNum(
              b[2],
              true
            )}</td>`;
            innerHtml += `<td class='text-right mr-1 pr-2 pb-4'>${
              ((Number(b[2]) * 100) / totalMarketValue).toFixed(2) + "%"
            }</td></tr>`;
          }
        });
        ...
        bodyLines.forEach(function (body, i) {
          ...
          if (j < 5 && selected[2] !== body[2]) {
            ...
            const td1 = `<td class='text-secondary flex flex-row items-center mx-2 ${margin}'>
                          <div class='w-2 h-2 rounded-full mr-2.5 border-solid' style="background-color: ${
                            CONSTANT["COIN_COLORS"][
                              body[0]["label"].replace("-USD", "")
                            ][0]
                          }"></div>${body[0]["label"].replace("-USD", "")}
                    </td>`;
            const td2 = `<td class='text-secondary text-right items-center pr-5 mx-1 ${margin}'>${formattedNum(
              body[2],
              true
            )}</td>`;
            const td3 = `<td class='flex flex-row justify-end mx-2 ${margin}'>${
              ((Number(body[2]) * 100) / totalMarketValue).toFixed(2) + "%"
            }</td>`;
        ...
        curContextObj.chart.config._config.data.datasets[
          ...
          CONSTANT["COIN_COLORS"][selected[0]["label"].replace("-USD", "")][0];
        if (selectedCoin === "All Coins") {
          ...
          innerHtml += `<td class='text-secondary text-right pr-5 mx-1 mt-1 pb-4'>${formattedNum(
            otherValue,
            true
          )}</td>`;
          innerHtml += `<td class='flex flex-row justify-end mx-2 mt-1 pb-4'>${
            ((Number(otherValue) * 100) / totalMarketValue).toFixed(2) + "%"
          }</td></tr>`;
          ...
          innerHtml += `<td class='text-secondary text-right m-1 pr-5'>${formattedNum(
            totalMarketValue,
            true
          )}</td>`;
